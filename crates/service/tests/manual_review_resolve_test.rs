...
async fn clear_resolution_pins(pool: &PgPool, order_id: &str) {
    sqlx::query(
        "UPDATE orders SET paykit_request_reference = NULL, paykit_invoice_id = NULL, \
         paykit_stack_id = NULL, paykit_stack_endpoint = NULL WHERE id = $1",
    )
    .bind(Uuid::parse_str(order_id).unwrap())
    .execute(pool)
    .await
    .expect("clear resolution pins");
}
...
async fn unpinned_manual_review_api_refuses_without_creating_outbox(pool: PgPool) {
    ...
    let buyer = new_actor(&app).await;
    let (order_id, _reference) = into_manual_review_held(&app, &paykit, &seller, &buyer).await;
    clear_resolution_pins(&pool, &order_id).await;
