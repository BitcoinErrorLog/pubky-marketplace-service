...
| `LIVE_TEST_SELLER_ALLOWLIST` | unset | comma-separated 52-character z-base-32 seller pubkys allowed to bind `bitcoin`/`stripe`/`paypal`. Unset or blank = all sellers (current production). Malformed entries fail process start. |
| `LIVE_TEST_MAX_USD_MINOR` | unset | when set (≥ 1), refuse bind of USD/2 orders whose `total_minor` exceeds this cap |
